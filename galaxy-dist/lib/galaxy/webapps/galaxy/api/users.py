...
class UserAPIController( BaseAPIController, UsesTagsMixin, CreatesUsersMixin, CreatesApiKeysMixin ):
    ...
    def anon_user_api_value( self, trans ):
        ...
        #NIK
        retval = subprocess.check_output("df -h /data | tail -1", shell=True)
        device, size, used, available, percent, mountpoint = retval.split()
        rval = used + " / " + size
        ...
        return {'total_disk_usage': 7777777777,
                'nice_total_disk_usage': rval,
                'quota_percent': percent}
